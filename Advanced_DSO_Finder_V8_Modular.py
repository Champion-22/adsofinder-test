...
try:
    ...
    # This error suggests libraries are missing from the Python environment
    st.error(f"Import Error: Missing libraries. Please install required packages (e.g., pip install -r requirements.txt). Details: {e}")
    st.stop()
...
try:
    ...
    st.error(f"Module Not Found Error: Could not find a required file ({e}). Ensure 'localization.py', 'astro_calculations.py', and 'data_handling.py' are in the correct directory.")
    st.stop()
...
def create_moon_phase_svg(illumination: float, size: int = 100) -> str:
    ...
    if not 0 <= illumination <= 1:
        print(f"Warn: Invalid moon illumination ({illumination}). Clamping.")
    ...
    if illumination < 0.01: pass
        ...
        if illumination <= 0.5: lae=0; se=1; lac=0; sc=1; d=(f"M {cx},{cy-radius} A {rx},{radius} 0 {lae},{se} {cx},{cy+radius} A {radius},{radius} 0 {lac},{sc} {cx},{cy-radius} Z")
        else: lac=1; sc=1; lae=1; se=1; d=(f"M {cx},{cy-radius} A {radius},{radius} 0 {lac},{sc} {cx},{cy+radius} A {rx},{radius} 0 {lae},{se} {cx},{cy-radius} Z")
...
def create_plot(plot_data: dict, min_altitude_deg: float, max_altitude_deg: float, plot_type: str, t: dict) -> plt.Figure | None:
    ...
    try:
        ...
        if plot_type == 'Altitude Plot':
            ...
            t_del=times.jd.max()-times.jd.min(); t_norm=(times.jd-times.jd.min())/(t_del+1e-9); colors=plt.cm.plasma(t_norm)
        ...
        return fig
    except Exception as e: st.error(f"Plot Err: Unexpected: {e}"); traceback.print_exc(); if fig: plt.close(fig); return None
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander(t.get('location_expander', "📍 Location"), expanded=True):
            ...
            if st.session_state.location_choice_key == "Manual":
                ...
                if search_submitted and st.session_state.location_search_query:
                    ...
                    with st.spinner(t.get('spinner_geocoding', "Searching...")):
                        try: geo=Nominatim(user_agent=agent); loc=geo.geocode(query, timeout=10); service="Nominatim"
                        ...
                        if not loc:
                           try: fgeo=ArcGIS(timeout=15); loc=fgeo.geocode(query,timeout=15); service="ArcGIS"
                           except (GeocoderTimedOut, GeocoderServiceError) as e2: print(f"ArcGIS fail: {e2}"); status_ph.info(t.get('location_search_info_fallback2', "Fallback 2..."))
                           except Exception as e2: print(f"ArcGIS error: {e2}"); status_ph.info(t.get('location_search_info_fallback2', "Fallback 2...")); err=e2
                        if not loc:
                           try: fgeo2=Photon(user_agent=agent, timeout=15); loc=fgeo2.geocode(query,timeout=15); service="Photon"
            ...
            if loc_valid_tz and lat is not None and lon is not None:
                if tf:
                    ...
                    # Corrected f-string for invalid timezone
                    except pytz.UnknownTimeZoneError: st.session_state.selected_timezone='UTC'; tz_msg = t.get('timezone_auto_fail_label','TZ:') + f" **UTC** (Invalid: '{found_tz}')"
        ...
        with st.expander(t.get('time_expander', "⏱️ Time"), expanded=False):
            ...
            is_now_time = (st.session_state.time_choice_exp == "Now") # Renamed variable to avoid conflict
            if is_now_time: st.caption(f"UTC: {Time.now().iso}")
        ...
        with st.expander(t.get('filters_expander', "✨ Filters"), expanded=False):
            ...
            min_alt_filt, max_alt_filt = st.session_state.min_alt_slider, st.session_state.max_alt_slider # Renamed variables
            ...
            all_types_list = [] # Renamed variable
            ...
            if all_types_list:
                curr_sel_types = [s for s in st.session_state.object_type_filter_exp if s in all_types_list] # Renamed variable
            ...
            if size_ok:
                try:
                    ...
                    st.slider(t.get('size_filter_label',"Size (arcmin):"), min_s, max_s, step=step, format="%.1f", key='size_arcmin_range', help=t.get('size_filter_help',"..."), disabled=size_disabled)
    ...
    time_disp = ""; is_now_main = (st.session_state.time_choice_exp == "Now") # Renamed variable
    if is_now_main:
        ref_time = Time.now()
        ...
        except Exception: time_disp = t.get('search_params_time_now',"Now (from {} UTC)").format(ref_time.to_datetime(timezone.utc).strftime('%Y-%m-%d %H:%M:%S') + " UTC") # Corrected format
    ...
    min_alt_disp, max_alt_disp = st.session_state.min_alt_slider, st.session_state.max_alt_slider; sel_types = st.session_state.object_type_filter_exp # Renamed variables
    ...
    direction_disp = st.session_state.selected_peak_direction # Renamed variable
    ...
    if find_clicked:
        ...
        if observer_run and df_catalog_data is not None:
            with st.spinner(t.get('spinner_searching',"Searching...")):
                try:
                    start_t, end_t, win_stat = astro_calculations.get_observable_window(observer_run, ref_time, is_now_main, t) # Use is_now_main
                    ...
                    if start_t and end_t and start_t < end_t:
                        ...
                        if filt_df.empty: results_ph.warning(t.get('warning_no_objects_found',"No objects found.") + " (init filt)"); st.session_state.last_results=[]
                            ...
                            final = []
                            sel_dir = st.session_state.selected_peak_direction; max_alt_filt_loop = st.session_state.max_alt_slider # Renamed variable
                            for obj in found:
                                if obj.get('Max Altitude (°)',-999) > max_alt_filt_loop: continue
    ...
    if st.session_state.last_results:
        ...
        if obs_exists and isinstance(win_start, Time) and isinstance(win_end, Time):
            ...
            try:
                illum=moon_illumination(mid_t); moon_pct=illum*100; moon_svg=create_moon_phase_svg(illum, size=50)
        ...
        for i, obj in enumerate(results_data):
            ...
            with obj_c.expander(title, expanded=is_exp):
                ...
                c2.markdown(t.get('results_max_alt_header',"**Max Alt:**")); max_a=obj.get('Max Altitude (°)',0); az=obj.get('Azimuth at Max (°)',0); direction=obj.get('Direction at Max','?'); az_fmt=t.get('results_azimuth_label',"(Az:{:.1f}°)").format(az); dir_fmt=t.get('results_direction_label',",Dir:{}").format(direction); c2.markdown(f"**{max_a:.1f}°** {az_fmt}{dir_fmt}")
                ...
                gq=urllib.parse.quote_plus(f"{name} astronomy"); gu=f"https://google.com/search?q={gq}"; c3.markdown(f"[{t.get('google_link_text','Google')}]({gu})", unsafe_allow_html=True) # Corrected f-string usage
                sq=urllib.parse.quote_plus(name); su=f"http://simbad.cds.unistra.fr/simbad/sim-basic?Ident={sq}"; c3.markdown(f"[{t.get('simbad_link_text','SIMBAD')}]({su})", unsafe_allow_html=True) # Corrected f-string usage
                ...
                if st.session_state.show_plot and st.session_state.plot_object_name == name:
                    p_data=st.session_state.active_result_plot_data; min_a=st.session_state.min_alt_slider; max_a=st.session_state.max_alt_slider; st.markdown("---")
                    with st.spinner(t.get('results_spinner_plotting',"Plotting...")):
                        try: fig = create_plot(p_data, min_a, max_a, st.session_state.plot_type_selection, t)
        ...
        if results_data:
            ...
            try:
                ...
                for obj in results_data:
                    ...
                    local_t, _ = get_local_time_str(peak_utc, st.session_state.selected_timezone)
                    export_data.append({
                        ...
                        t.get('results_export_time_max_local', "Time at Max (Local TZ)"): local_t,
    ...
    with st.expander(t.get('custom_target_expander',"Plot Custom Target")):
        ...
        if st.session_state.show_custom_plot and st.session_state.custom_target_plot_data:
            c_data=st.session_state.custom_target_plot_data; min_a=st.session_state.min_alt_slider; max_a=st.session_state.max_alt_slider
            with plot_area.container():
                 st.markdown("---")
                 with st.spinner(t.get('results_spinner_plotting',"Plotting...")):
                     try: fig=create_plot(c_data, min_a, max_a, st.session_state.plot_type_selection, t)
                     except Exception as e: st.error(f"Plot Err:{e}"); traceback.print_exc(); fig=None
