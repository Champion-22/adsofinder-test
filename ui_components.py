        ...
        with st.expander(t.get('location_expander', "📍 Standort"), expanded=True):
            ...
            if loc_valid_for_tz_lookup and lat is not None and lon is not None:
                if tf:
                    ...
                    except Exception as e: print(f"Fehler bei Zeitzonen-Suche: {e}"); found_timezone_val = None # Handle potential tf errors
...
def display_results(t: dict, results_ph: st.container, observer_run: Observer | None) -> None:
    ...
    if observer_exists and isinstance(window_start, Time) and isinstance(window_end, Time):
        ...
        except Exception as e: results_ph.warning(t.get('moon_phase_error',"Mondphase konnte nicht berechnet werden: {}").format(e)); print(f"Fehler bei Mondphasenberechnung: {e}"); moon_svg_icon = None # Handle error
        if moon_svg_icon:
...
def create_manual_cosmology_calculator(t: dict) -> None:
    ...
    with st.expander(t.get('manual_cosmology_expander', "🌌 Manueller Kosmologie-Rechner")):
        ...
        with col2:
            # Use t.get() for the markdown text
            st.markdown(f"**{t.get('cosmo_params', 'Kosmologische Parameter')}**")
            # Use t.get() for labels
            h0_manual_input = st.number_input(label=t.get("hubble_h0", "Hubble-Konstante (H₀) [km/s/Mpc]"), min_value=1.0, value=st.session_state.get("manual_h0_value", H0_DEFAULT), step=0.1, format="%.1f", key="manual_h0_value")
            omega_m_manual_input = st.number_input(label=t.get("omega_m", "Materiedichte (Ωm)"), min_value=0.0, max_value=2.0, value=st.session_state.get("manual_omega_m_value", OMEGA_M_DEFAULT), step=0.01, format="%.3f", key="manual_omega_m_value")
            omega_lambda_manual_input = st.number_input(label=t.get("omega_lambda", "Dunkle Energie (ΩΛ)"), min_value=0.0, max_value=2.0, value=st.session_state.get("manual_omega_lambda_value", OMEGA_LAMBDA_DEFAULT), step=0.01, format="%.3f", key="manual_omega_lambda_value")
        ...
        st.markdown("---")
        # Use t.get() for the subheader, ensure formatting works with .get()
        st.subheader(t.get("results_for", "Ergebnisse für z = {z:.5f}").format(z=z_manual_input))
        ...
        if error_key_manual:
            ...
            # Use t.get() for error messages
            error_text_manual = t.get(error_key_manual, error_key_manual).format(**error_args_manual) # Provide key as default
            ...
            # Use t.get() for labels and units
            st.metric(label=t.get("lookback_time", "Rückblickzeit"), value=f"{lookback_gyr_res_man:.4f}", delta=t.get("unit_Gyr", "Gyr"))
            lookback_example_key_man = get_lookback_comparison(lookback_gyr_res_man); st.caption(f"*{t.get(lookback_example_key_man, '')}*") # Use t.get()

            st.markdown("---")
            st.subheader(t.get("cosmo_distances", "Kosmologische Distanzen"))
            res_col1, res_col2 = st.columns(2)

            with res_col1:
                st.markdown(t.get("comoving_distance_title", "**Mitbewegte Distanz:**")); st.text(f"  {comoving_mpc_res_man:,.4f} {t.get('unit_Mpc', 'Mpc')}"); st.text(f"  {comoving_gly_res_man:,.4f} {t.get('unit_Gly', 'Gly')}"); comoving_example_key_man = get_comoving_comparison(comoving_mpc_res_man); st.caption(f"*{t.get(comoving_example_key_man, '')}*")
                st.text(f"  {comoving_km_res_man:,.3e} {t.get('unit_km_sci', 'km (wiss.)')}"); st.text(f"  {comoving_km_ausgeschrieben_man} {t.get('unit_km_full', 'km (ausgeschr.)')}"); st.text(f"  {comoving_ly_res_man:,.3e} {t.get('unit_LJ', 'LJ')}")
            with res_col2:
                st.markdown(t.get("luminosity_distance_title", "**Leuchtkraftdistanz:**")); st.text(f"  {luminosity_mpc_res_man:,.4f} {t.get('unit_Mpc', 'Mpc')}"); st.text(f"  {luminosity_gly_res_man:,.4f} {t.get('unit_Gly', 'Gly')}"); st.caption(f"*{t.get('explanation_luminosity', '')}*")
                st.markdown(t.get("angular_diameter_distance_title", "**Winkeldurchmesserdistanz:**"), unsafe_allow_html=True); st.text(f"  {ang_diam_mpc_res_man:,.4f} {t.get('unit_Mpc', 'Mpc')}"); st.text(f"  {ang_diam_gly_res_man:,.4f} {t.get('unit_Gly', 'Gly')}"); st.caption(f"*{t.get('explanation_angular', '')}*")

            integration_warn_key_man = cosmo_results_manual.get('integration_warning_key')
            if integration_warn_key_man: integration_warn_args_man = cosmo_results_manual.get('integration_warning_args', {}); st.caption(t.get(integration_warn_key_man, "...").format(**integration_warn_args_man))
            st.caption(t.get("calculation_note", "...")) # Use t.get()
